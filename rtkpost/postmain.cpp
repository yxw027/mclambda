    ...
    : TForm(Owner)
{
    ...
    DynamicModel=IonoOpt=TropOpt=RovAntPcv=RefAntPcv=AmbRes=MCoptions=0;
                                    ...
                                    filopt_t &filopt)
{
    ...
    if (PosMode==PMODE_SINGLE||PosMode==PMODE_MOVEB||PosMode==PMODE_MOVEB_MCLAMBDA) {
...
void __fastcall TMainForm::LoadOpt(void)
{
    ...
    MCoptions          =ini->ReadInteger("opt","mcoptions",      1);
...
void __fastcall TMainForm::SaveOpt(void)
{
    ...
    ini->WriteInteger("opt","mcoptions",   MCoptions    );
