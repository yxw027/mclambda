                      ...
                      const solopt_t *sopt)
{
    ...
    if (PMODE_DGPS<=popt->mode&&popt->mode<=PMODE_FIXED&&popt->mode!=PMODE_MOVEB&&popt->mode!=PMODE_MOVEB_MCLAMBDA) {
...
static void combres(FILE *fp, const prcopt_t *popt, const solopt_t *sopt)
{
    ...
    for (i=0,j=isolb-1;i<isolf&&j>=0;i++,j--) {
        ...
        if ((tt=timediff(solf[i].time,solb[j].time))<-DTTOL) {
            ...
            if ((popt->mode==PMODE_KINEMA||popt->mode==PMODE_MOVEB||popt->mode==PMODE_MOVEB_MCLAMBDA)&&
                sols.stat==SOLQ_FIX) {
            ...
            if (popt->mode==PMODE_MOVEB||popt->mode==PMODE_MOVEB_MCLAMBDA) {
